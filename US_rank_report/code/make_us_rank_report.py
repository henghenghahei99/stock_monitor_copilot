...
def main() -> None:
    ...
    rank_note = ("<p class='note' style='color:#555;background:#f5f7fa;border-radius:8px;padding:10px 14px;font-size:12px;line-height:1.8'>"
                 ...
                 f"⑤ 总分=趋势分+动量分（两个分值已按当日权重加权，即实际入总分的**贡献值**；权重按当日池内两分量的平均绝对量级取反比确定，"
                 f"使两者对总分的平均贡献相等，w_M 限[35%,65%]；今日 {w_txt}。原始未加权指标仅供下方走势图）；入池=原行业得分前{top} ∪ 动量入池分前{top}(并集, 最多{2 * top})，池内按总分降序排名；"
