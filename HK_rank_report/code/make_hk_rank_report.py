...
# -*- coding: utf-8 -*-
"""
港股板块 A_rank HTML/TXT 日报 —— 版式与 A股日报(a_rank_report) 对齐

读 output/hk_rank_{YYYYMMDD}.csv(入池排名) + hk_delta_{YYYYMMDD}.csv(升降,可选)
多日(hk_rank_*.csv)自动生成 4 张走势图(动量↑/↓、趋势↑/↓), 与 A股一致放报告末尾。
用法: python make_hk_rank_report.py [YYYYMMDD] [--top 7]
"""
...
def _dynamic_weights(trend, mom, lo: float = 0.35, hi: float = 0.65) -> tuple[float, float]:
    """池内动态权重(与 hk_rank.dynamic_weights 同一规则): 按两分量平均绝对量级取反比,
...
def _add_eval(delta: pd.DataFrame, rank: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in d.iterrows():
        ...
        # **所有状态**都用“池内口径”评价: 趋势分变化(增强/减弱/平稳) +
        # 动量分变化(≥1.5爆发/0~1.5增强/-1.5~0减弱/<-1.5大幅下滑), 阈值按加权后分值。
        # 仅当“前日无分数”(变化列缺失)时, 才退回“当日分数在池内百分位”的评价。
        if tw or mw:
...
def _load_series(tag: str, days: int = CHART_DAYS) -> list[dict]:
    """从 hk_rank_*.csv 取 <= tag 的最近 days 天: 每天 入池/分数。"""
    cand = []
    for fn in os.listdir(OUT):
        if not (fn.startswith("hk_rank_") and fn.endswith(".csv")):
            continue
        core = fn[len("hk_rank_"):-4]
        if core.isdigit() and core <= tag:
            cand.append(core)
    cand = sorted(set(cand))[-days:]
    series = []
    for core in cand:
        p = os.path.join(OUT, f"hk_rank_{core}.csv")
        try:
...
def main() -> None:
    ...
    a = p.parse_args()
    tag = a.day or os.path.basename(sorted(glob.glob(os.path.join(OUT, "hk_rank_*.csv")))[-1]) \
        .replace("hk_rank_", "").replace(".csv", "")
    rank = pd.read_csv(os.path.join(OUT, f"hk_rank_{tag}.csv"), encoding="utf-8-sig")
    disp = f"2026-{tag[4:6]}-{tag[6:]}"
    top = a.top
    title = f"港股板块 A_rank 日报 V2 · {disp}"
    ...
    _sc = 2.0   # 与 hk_rank.DISPLAY_SCALE 一致
    sc_txt = f"原始指标×{_wt * _sc:.2f} / ×{_wm * _sc:.2f}"

    delta_html = ""
    delta_txt_lines: list[str] = []
    delta_note = ""
    series = _load_series(tag)
    dpath = os.path.join(OUT, f"hk_delta_{tag}.csv")
    if os.path.exists(dpath):
        ...
        if "排名变化" in d.columns and "状态" in d.columns:
            ...
            # 组内次键: 当日动量分降序(新进池/退出池按动量强弱; 池内同动量变化时按动量强弱)
            _gm = {"完全新进池": 0, "新进池": 1, "池内": 2, "退出池": 3}
            d["_g"] = d["状态"].astype(str).map(lambda s: _gm.get(s, 9))
            _pm = pd.to_numeric(d["动量分变化"], errors="coerce").fillna(0)
            d["_pm"] = _pm.where(d["状态"].astype(str).eq("池内"), 0.0)
            d["_mc"] = pd.to_numeric(
                d["行业"].map(lambda i: mom_today.get(str(i).strip(), -999.0)),
                errors="coerce").fillna(-999.0)
            d = (d.sort_values(["_g", "_pm", "_mc"], ascending=[True, False, False], kind="stable")
        ...
        if not d.empty:
            ...
            delta_note = ("<p class='note' style='color:#555;background:#f5f7fa;border-radius:8px;"
                          ...
                          f"(入池=原得分前{top} ∪ 动量入池分前{top}、池内按总分=趋势分+动量分[两分值已按各日权重加权] 降序)计算后对比。"
                          ...
                          "评价：**所有状态**统一按池内口径(趋势分变化+动量分变化)；仅前日无分数时退回当日分数在今日池内百分位(趋势 前10%很强 / 动量 前10%爆发 / 10-30%强 / 30-70%一般 / 70-100%弱)。</p>")
    ...
    if len(series) >= 2 and today_pool:
        ...
        gi = 0
        chart_no = 0
        png_dir = os.path.join(OUT, f"hk_rank_report_{tag}_charts")
    ...
    rank_note = ("<p class='note' style='color:#555;background:#f5f7fa;border-radius:8px;padding:10px 14px;font-size:12px;line-height:1.8'>"
                 "表后注(今日板块排名算法)：<br>"
                 "① 个股趋势9条件【V2=7交易日窗口】(站上MA7 / MA7上行 / 低点抬高(近3日低>前4日低) / 高点抬高(近3日高>前4日高) / "
                 "斜率向上(近7日回归) / 近7日新高(≥近7日高×98%) / 放量(近3日均量>近7日均量) / 7窗口持续性[7日为窗逐日前移共7窗, ≥6窗涨幅>0 且 ≥4窗涨幅>2%] / "
                 "7窗口量能放大[7窗累计成交量回归斜率>0 且 最新窗量>最老窗量])满足数≥6命中；历史K线不足只评得部分条件时按比例折算到 /8。<br>"
                 ...
                 "◆紫=相比前日新进入动量前7。<br>"
                 "⑦ 板块=东财港股行业(港股行业体系, 共31类)：由东财港股F10公司概况(BELONG_INDUSTRY)拉取, "
                 "缓存 data/hk_em_industry.json；参与股票不含人民币柜台(8xxxx)重复代码。<br>"
                 "⑧ 数量因子f(沿用A股口径): 从0只起按0.003/只(=0.06/20)连续线性递减，n≥120封底0.64不再减(如20只≈0.94、100只=0.70)；"
                 "计数=该行业通过参与门槛(近5日均成交额>4000万港元)的股票数；趋势分、动量分(±10)、动量入池分及入池资格(得分/动量两条腿)均乘f。</p>")
    if SHOW_NOTES:
    ...
    html_path = os.path.join(OUT, f"hk_rank_report_{tag}.html")
    ...
    txt_path = os.path.join(OUT, f"hk_rank_report_{tag}.txt")
