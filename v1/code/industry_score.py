...
def aggregate(df: pd.DataFrame, col: str, mapping: dict[int, int]) -> pd.DataFrame:
    ...
    df = df[df["加权分"] > 0]  # 剔除6分以下(0分)
    if df.empty:
        return df

    # 代表股: 每个行业加权分最高的前 N_REP 只(同分按20日涨幅降序), 不足则给全部
    N_REP = 9
    rep = (df.sort_values(["加权分", "chg20"], ascending=[False, False])
             .groupby("industry", sort=False).head(N_REP))
