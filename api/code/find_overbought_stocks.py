...
DEFAULT_MIN_PRICE = 5.0            # 剔除低于该价格的仙股
...
def http_get(url: str, timeout: int = 15) -> bytes:
    """带重试的 HTTP GET(指数退避, 应对腾讯/东财临时限流)。"""
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                time.sleep(3 * (2 ** (attempt - 1)))  # 3s, 6s, 12s, 24s ...
...
def tencent_kline(
    ...
    prefixed 形如 usAAPL.OQ / sh600519 / hk00700。带本地缓存, 减少请求避免限流。
    """
    cache_file = None
    if use_cache:
        ...
        if os.path.exists(cache_file) and (
            time.time() - os.path.getmtime(cache_file)
        ) < KLINE_CACHE_TTL_DAYS * 86400:
            try:
                with open(cache_file, encoding="utf-8") as f:
                    return _kline_rows_to_series(json.load(f))
            except Exception:  # noqa: BLE001
                pass
