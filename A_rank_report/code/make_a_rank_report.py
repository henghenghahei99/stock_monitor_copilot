...
def main() -> None:
    ...
    rank_note = ("<p class='note'>表后注(今日板块排名算法)：<br>"
                 ...
                 f"w_M 限 [35%, 65%]；今日 {w_txt}）；入池=原行业得分前{args.top}名 ∪ 动量入池分前{args.top}名(并集, 最多{2 * args.top}个)，"
    ...
    if rk is not None:
        ...
        lines.append("算法: ①个股分=上涨趋势8条件(多头排列/站上年线/MA20上行/低点抬高/高点抬高/斜率向上/"
                     ...
                     f"⑤总分=趋势分/动量分按当日池内量级动态配平(平均贡献相等, w_M限[35%,65%]; 今日{w_txt}), 入池=原得分前{args.top} ∪ 动量入池分前{args.top}(并集, 最多{2 * args.top}), 池内按总分降序; "
    ...
    if delta is not None:
        ...
        lines.append(f"算法: 前一日与今日各自按上方A_rank(入池=原得分前{args.top} ∪ 动量入池分前{args.top}, 池内按总分=趋势分/动量分动态配平降序)后对比; "
