...
def main() -> None:
    ...
    # 升降表新增“5日进池次数”: 窗口内(含今日, 最多5个交易日)该板块在池的交易日数;
    # 插在“评价”列前面(池口径与走势图一致: 当日 原得分前top ∪ 动量入池分前top)
    if delta is not None and series:
        _pool_cnt: dict[str, int] = {}
        for _p in series:
            for _i in _p["pool"]:
                _k = str(_i).strip()
                _pool_cnt[_k] = _pool_cnt.get(_k, 0) + 1
        _cnt_vals = [int(_pool_cnt.get(str(_i).strip(), 0)) for _i in delta["行业"]]
        _loc = int(delta.columns.get_loc("评价")) if "评价" in delta.columns else len(delta.columns)
        delta.insert(_loc, "5日进池次数", _cnt_vals)
    ...
    delta_note = (f"<p class='note'>表后注(排名升降算法)：对前一交易日与今日各自按上方A_rank"
                  ...
                  f"5日进池次数=最近{len(series) or 5}个交易日(含今日, 与下方走势图同一窗口)该板块在池的天数(池口径=原得分前{args.top} ∪ 动量入池分前{args.top})；"
    ...
    if delta is not None:
        ...
        lines.append(f"算法: 前一日与今日各自按上方A_rank(入池=原得分前{args.top} ∪ 动量入池分前{args.top}, 池内按总分=趋势分+动量分, 两分值已加权)后对比; "
                     ...
                     f"5日进池次数=最近{len(series) or 5}个交易日(含今日)该板块在池的天数(池口径同上, 每日取当天自己的池); "
