...
def main() -> None:
    ...
    rank_note = ("<p class='note'>表后注(今日板块排名算法)：<br>"
                 ...
                 "(不足按实际只数)，S=Σ前10的m；动量入池分=S；展示动量分=S÷(5/3×动量股数) 归一(允许为负, ±10封顶)。<br>"
                 ...
                 "⑥ 代表股=趋势前5(加权分最高, 记X/8,+近20日涨幅) + ◎动量前5(板块全部成分股按m最强, 橙色◎=短线动量, 括号=当日涨幅)。<br>"
    ...
    if rk is not None:
        ...
        lines.append("算法: ①个股分=上涨趋势8条件(多头排列/站上年线/MA20上行/低点抬高/高点抬高/斜率向上/"
                     ...
                     "(不足按实际只数)S=Σ, 动量入池分=S, 展示动量分=S/(5/3×动量股数)(允许为负, ±10封顶); "
